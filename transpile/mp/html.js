...
const ENGINE_LOADERS_MAP = {
  pug: {
    ...
    renderKey: 'data',
    deps: ['pug', 'html-loader', 'jade-html-loader']
  },
  ejs: {
    htmlAssit: true,
...
module.exports = function (config, isDevelopment) {
  ...
  if (IndexFiles.length === 1) {
    Plugins.push(new HtmlWebpackPlugin({
      filename: Path.posix.join(CONFIG_HTML.output, `${Path.basename(IndexFiles[0],`.${EXTTYPE}`)}.html`),
    ...
    IndexFiles.forEach(file => {
      Plugins.push(new HtmlWebpackPlugin({
        filename: Path.posix.join(CONFIG_HTML.output, `${Path.basename(file,`.${EXTTYPE}`)}.html`),
        template: file,
        // 多页应用关闭自动注入
        inject: false,
        xhtml: false,
