...
function GetLoaders(config, isDevelopment, ext) {
  ...
  if (CONFIG_STYLE.sprites) {
    ...
    const PostcssSpritesOpts = `{
      ...
      relativeTo: 'rule',
      spritePath: process.env.BOI_ENV === 'dev'?'${Path.posix.join(config.basic.output, config.image.output)}':'./',
      stylesheetPath: process.env.BOI_ENV === 'dev'?'${Path.posix.join(config.basic.output, CONFIG_STYLE.output)}':null,
      spritesmith: {
        padding: 5,
      },
      groupBy: image => {
...
/**
 * @module boi-tranpiler/generateModuleAndPluginsForStyleCompile
...
module.exports = function (config, isDevelopment, deployConf = null) {
  ...
  const {
    loaders,
    deps
  } = GetLoaders(config, isDevelopment, EXTTYPE);
